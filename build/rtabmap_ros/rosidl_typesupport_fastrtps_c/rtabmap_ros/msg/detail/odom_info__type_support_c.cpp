...
#include "geometry_msgs/msg/detail/transform__functions.h"  // guess, local_bundle_model_transforms, transform, transform_filtered, transform_ground_truth
...
#include "sensor_msgs/msg/detail/camera_info__functions.h"  // local_bundle_models
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  // Field name: local_bundle_model_transforms
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
    ...
    auto array_ptr = ros_message->local_bundle_model_transforms.data;
    cdr << static_cast<uint32_t>(size);
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_serialize(
          &array_ptr[i], cdr))
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: local_bundle_models
  {
    ...
    if (ros_message->local_bundle_models.data) {
      sensor_msgs__msg__CameraInfo__Sequence__fini(&ros_message->local_bundle_models);
    }
  ...
  // Field name: local_bundle_model_transforms
  {
    ...
    cdr >> cdrSize;
    size_t size = static_cast<size_t>(cdrSize);
    if (ros_message->local_bundle_model_transforms.data) {
      geometry_msgs__msg__Transform__Sequence__fini(&ros_message->local_bundle_model_transforms);
    }
    if (!geometry_msgs__msg__Transform__Sequence__init(&ros_message->local_bundle_model_transforms, size)) {
      return "failed to create array for field 'local_bundle_model_transforms'";
    }
    auto array_ptr = ros_message->local_bundle_model_transforms.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
